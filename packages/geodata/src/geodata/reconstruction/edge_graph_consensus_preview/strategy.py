"""Valhalla edge-graph consensus reconstruction strategy."""
...
def _path_score(
    ...
    score += transition_weight * sum(
        edge_weights[(path[idx], path[idx + 1])]
...
def _consensus_path(
    ...
    ranked_starts = sorted(
        ...
        reverse=True,
    )[: max(1, start_candidates)]

    beam: list[tuple[float, tuple[str, ...]]] = []
    for start in ranked_starts:
        beam.append((
            _path_score((start,), node_weights, edge_weights, transition_weight),
            (start,),
        ))
    ...
    while beam:
        ...
        for score, path in beam:
            ...
            for successor in sorted(
                ...
                key=lambda node_id: (edge_weights[(path[-1], node_id)], node_weights[node_id]),
                reverse=True,
            ):
...
class EdgeGraphConsensusPreviewStrategy:
    """Consensus route from Valhalla-matched edge sequences."""

    key: str = "edge_graph_consensus_preview"
    label: str = "Valhalla edge-graph consensus (preview)"
    ...
    def reconstruct(
        ...
        start_candidates = max(1, int(effective_params.get("start_candidates", 5)))
        ...
        for trace in traces:
            ...
            trace_edge_ids = _collapse_consecutive([_edge_key(edge) for edge in result.edges])
            if not trace_edge_ids:
                continue

            matched_trace_count += 1
            total_matched_edges += len(trace_edge_ids)
            node_weights.update(trace_edge_ids)
            edge_weights.update(zip(trace_edge_ids, trace_edge_ids[1:], strict=False))

            for edge in result.edges:
                edge_id = _edge_key(edge)
                geometry = _edge_geometry(edge, result.shape_coords)
                if len(geometry) >= 2:
        ...
        diagnostics: dict[str, int | float | str] = {
            ...
            "trace_count": len(traces),
            "matched_trace_count": matched_trace_count,
            "graph_node_count": len(node_weights),
            "graph_edge_count": len(edge_weights),
            "total_matched_edges": total_matched_edges,
            "consensus_edge_count": len(consensus_edge_ids),
            "consensus_route_points": len(route_coordinates),
            "costing": costing,
            "search_radius": search_radius,
            "gps_accuracy": gps_accuracy,
            "beam_width": beam_width,
            "start_candidates": start_candidates,
            "transition_weight": transition_weight,
            "consensus_method": "beam_search_weighted_path",
        }
